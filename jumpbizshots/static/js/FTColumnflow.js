...
var FTColumnflow = (function () {
	...
	var
		...
		+ '#[targetId] .[pageClass] { position: relative; width: [viewportWidth]px; height: [viewportHeight]px; [pageArrangement] }\n'
		+ '#[targetId] .[columnClass] { position: absolute; width: [columnWidth]px; overflow: hidden; }\n'
		+ '#[targetId] .[pageClass] .[fixedElementClassName] { position: relative; }\n'
	...
	function FTColumnflow(target, viewport, userConfig) {
		...
		function _setConfig(userConfig) {
			...
			config.pagePadding[0] = parseInt(config.pagePadding[0], 10);
			config.pagePadding[1] = parseInt(config.pagePadding[1], 10);
			if (isNaN(config.pagePadding[0]) || isNaN(config.pagePadding[1])) {
		...
		function _setLayoutDimensions() {
			...
			if ('horizontal' === config.pageArrangement) {
				config.layoutDimensions.pageInnerWidth  = config.viewportWidth - (2 * config.pagePadding[0]);
				config.layoutDimensions.pageInnerHeight = config.viewportHeight - (2 * config.pagePadding[1]);
				config.layoutDimensions.colDefaultTop   = config.pagePadding[1];
				config.layoutDimensions.colDefaultLeft  = config.pagePadding[0];
			} else {
				config.layoutDimensions.pageInnerWidth  = config.viewportWidth - (2 * config.pagePadding[0]);
				config.layoutDimensions.pageInnerHeight = config.viewportHeight - (2 * config.pagePadding[1]);
				config.layoutDimensions.colDefaultTop   = config.pagePadding[1];
